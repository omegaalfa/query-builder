...
final class PgVectorCacheKeyTest extends TestCase
{
    ...
    public function testLimitAndPaginationUseDifferentCacheKeys():void{$limit=$this->builder()->select('documents',['id'])->limit(10);$pagination=$this->builder()->select('documents',['id'])->paginate(10);self::assertSame($limit->getQuerySql(),$pagination->getQuerySql());self::assertNotSame($this->key($limit),$this->key($pagination));}
}
